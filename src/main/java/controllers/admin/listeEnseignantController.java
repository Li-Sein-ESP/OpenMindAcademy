...
public class listeEnseignantController {
    ...
    private void loadTeachers() {
        try {
            enseignantTable.getItems().setAll(userService.getUsersByRole(enums.Role.TEACHER));
        } catch (SQLException e) {
            NavigationUtil.showError("Erreur lors du chargement des enseignants : " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private void editUser(User user) throws IOException {
        if (user.getRole() != enums.Role.TEACHER) {
            NavigationUtil.showError("Seuls les enseignants peuvent être modifiés ici.");
            return;
        }
        ...
        Stage stage = (Stage) enseignantTable.getScene().getWindow();
        stage.setScene(scene);
        stage.show();
    }

    private void deleteUser(User user) {
        Alert confirmation = new Alert(Alert.AlertType.CONFIRMATION, "Voulez-vous vraiment supprimer cet enseignant ? Cette action est irréversible.", ButtonType.YES, ButtonType.NO);
